      ...
      <div className="flex flex-col md:flex-row items-center gap-4 bg-white p-4 rounded-2xl border-2 border-slate-200 shadow-sm">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full bg-slate-50 border border-slate-300 rounded-xl pl-12 pr-4 py-3.5 text-base font-bold text-slate-900 placeholder-slate-400 focus:outline-none focus:border-cyan-600"
          />
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          >
            {companies.map((c) => (
              <option key={c._id} value={c._id} className="bg-white text-slate-900 font-bold text-base py-1">
                {c.name} ({c.code})
              </option>
            ))}
          </select>
          ...
          <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-300">
            <button
              ...
              className={`p-2.5 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-cyan-600 text-white font-bold' : 'text-slate-600 hover:text-slate-900'}`}
            ...
            <button
              ...
              className={`p-2.5 rounded-lg transition-all ${viewMode === 'table' ? 'bg-cyan-600 text-white font-bold' : 'text-slate-600 hover:text-slate-900'}`}
              title="Table View"
            >
