...
export default function BlogsAdminPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem?._id) {
        await adminFetch(`/blogs/${editingItem._id}`, { method: 'PUT', body: JSON.stringify(payload) });
      } else {
  ...
  const changeStatus = async (id: string, newStatus: 'Draft' | 'Publish' | 'Hide') => {
    try {
      await adminFetch(`/blogs/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status: newStatus }) });
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Soft-delete this blog post?')) return;
    try {
  ...
  const getCompanyCode = (comp: string | { _id: string; name: string; code: string }) => {
    if (typeof comp === 'object' && comp?.code) return comp.code;
    const found = companies.find(c => c._id === comp);
    return found ? found.code : 'WEB';
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border-2 border-slate-200 shadow-sm">
        <div>
          <h1 className="text-xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Blog & Insights Management</h1>
          <p className="text-base text-slate-600 mt-1 font-bold">
            Articles and news for <span className="text-cyan-700 font-extrabold text-lg">{getCompanyName(companyFilter)}</span>
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white border-2 border-slate-200 rounded-2xl p-12 text-center text-slate-500 text-lg font-medium shadow-sm">
          No blog posts found for {getCompanyName(companyFilter)}.
        </div>
      ) : viewMode === 'grid' ? (
        /* CARD GRID VIEW */
                ...
                <div>
                  ...
                  <div className="relative aspect-video bg-gradient-to-br from-cyan-900 via-slate-900 to-blue-950 overflow-hidden border-b border-slate-200">
                    ...
                    <div className="absolute inset-0 p-5 flex flex-col justify-end bg-gradient-to-t from-slate-950/80 via-transparent to-transparent z-0">
                      <div className="text-[11px] font-black uppercase text-cyan-300 tracking-wider mb-1">{blog.category}</div>
                      <div className="font-extrabold text-sm line-clamp-2 text-white drop-shadow-sm">{blog.title}</div>
                    </div>
                    ...
                    <div className="absolute top-3 right-3 z-10">
                      <select
                        value={blog.status}
                        onChange={(e) => changeStatus(blog._id!, e.target.value as any)}
                  ...
                  <div className="p-6 space-y-3">
                    <div className="flex flex-wrap items-center gap-3 text-xs font-bold text-slate-500">
                      <span className="flex items-center gap-1">
                        <HiOutlineUser className="w-4 h-4 text-cyan-600" />
                      ...
                      {blog.readTime && (
                        <>
                          <span>•</span>
                          <span className="text-cyan-700 font-extrabold">{blog.readTime}</span>
                        </>
                      )}
                    </div>

                    <h3 className="font-extrabold text-xl text-slate-900 line-clamp-2 leading-snug hover:text-cyan-700 transition-colors">
                      <Link href={`/admin/blogs/${blog.slug || blog._id}`}>
                        {blog.title}
                      </Link>
                    </h3>

                    <p className="text-sm font-medium text-slate-600 line-clamp-3 leading-relaxed">
                      {blog.shortDescription || blog.excerpt || 'No summary provided.'}
                    </p>

                    {/* Highlights Preview */}
                    {blog.highlights && blog.highlights.length > 0 && (
                      <div className="bg-cyan-50/50 p-3 rounded-xl border border-cyan-100 space-y-1">
                        <span className="text-[10px] font-black uppercase text-cyan-800 tracking-wider block">Key Highlights</span>
                        <p className="text-xs font-bold text-slate-800 truncate">✓ {blog.highlights[0]}</p>
                      </div>
                    )}
                ...
                return (
                  <tr key={blog._id || blog.slug || blog.title} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-cyan-700 font-mono text-xs font-bold mt-0.5">/{blog.slug || blog.id}</div>
                        </div>
                      </div>
                    </td>
                    <td className="p-4">
                      ...
                      <p className="text-xs font-bold text-slate-500 mt-1.5">By {blog.author}</p>
                    </td>
                    <td className="p-4">
                      ...
                      {blog.readTime && <p className="text-xs font-extrabold text-cyan-700 mt-1">{blog.readTime}</p>}
                    </td>
                    ...
                    <td className="p-4 text-right space-x-2">
                      ...
                      <button onClick={() => handleOpenModal(blog)} className="p-2 rounded-xl bg-slate-100 text-slate-700 hover:text-cyan-700 hover:bg-cyan-50 border border-slate-200 transition-colors">
                        <HiOutlinePencil className="w-5 h-5" />
                      </button>
                      <button onClick={() => handleDelete(blog._id!)} className="p-2 rounded-xl bg-slate-100 text-slate-700 hover:text-red-600 hover:bg-red-50 border border-slate-200 transition-colors">
                        <HiOutlineTrash className="w-5 h-5" />
                      </button>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-start sm:items-center justify-center p-3 sm:p-6 overflow-y-auto pt-6 pb-12">
          <div className="bg-white border-2 border-slate-300 rounded-3xl max-w-3xl w-full flex flex-col shadow-2xl my-auto max-h-[90vh] overflow-hidden relative">
            ...
            <div className="p-6 sm:p-8 overflow-y-auto max-h-[calc(90vh-80px)] space-y-6">
              {/* Banner Image */}
              {(viewingBlog.featuredImage?.url || viewingBlog.image || viewingBlog.imageUrl) ? (
                <div className="w-full aspect-video rounded-2xl overflow-hidden border border-slate-200 shadow-md">
                  <img
                    src={viewingBlog.featuredImage?.url || viewingBlog.image || viewingBlog.imageUrl}
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm font-bold text-slate-600 pt-1 border-b border-slate-200 pb-4">
                  <span className="flex items-center gap-1.5">
                    ...
                    <span>Author: {viewingBlog.author} {viewingBlog.authorRole ? `(${viewingBlog.authorRole})` : ''}</span>
                  </span>
                  <span>•</span>
                  <span className="flex items-center gap-1.5">
                    <HiOutlineCalendar className="w-5 h-5 text-cyan-600" />
                    <span>Published: {viewingBlog.publishDate || viewingBlog.date}</span>
                  </span>
                  {viewingBlog.readTime && (
                    <>
                      <span>•</span>
                      <span className="bg-cyan-50 text-cyan-800 px-2.5 py-0.5 rounded-md text-xs font-black border border-cyan-200">
                        {viewingBlog.readTime}
                      </span>
                    </>
                  )}
                  <span>•</span>
                  <span className="font-mono text-cyan-700 text-xs">/{viewingBlog.slug || viewingBlog.id}</span>
                </div>
              </div>

              {/* Short Summary Box */}
              {(viewingBlog.shortDescription || viewingBlog.excerpt) && (
                <div className="bg-cyan-50/60 border-l-4 border-cyan-600 p-4 rounded-r-2xl text-slate-800 font-bold text-base italic leading-relaxed">
                  &ldquo;{viewingBlog.shortDescription || viewingBlog.excerpt}&rdquo;
                </div>
              )}

              {/* Key Highlights Box */}
              {viewingBlog.highlights && viewingBlog.highlights.length > 0 && (
                <div className="bg-slate-50 border-2 border-slate-200 p-5 rounded-2xl space-y-2.5">
                  ...
                  <ul className="space-y-1.5 pl-1">
                    {viewingBlog.highlights.map((item, idx) => (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-start sm:items-center justify-center p-3 sm:p-6 overflow-y-auto pt-6 pb-12">
          <div className="bg-white border-2 border-slate-300 rounded-3xl max-w-4xl w-full flex flex-col shadow-2xl my-auto max-h-[88vh] overflow-hidden relative">
            <div className="p-6 border-b border-slate-200 flex items-center justify-between bg-white sticky top-0 z-30 shadow-sm">
              <h3 className="font-extrabold text-2xl text-slate-900 tracking-tight">{editingItem ? 'Edit Blog Post' : 'Create Blog Post'}</h3>
              <button
                type="button"
            ...
            <form onSubmit={handleSave} className="p-6 space-y-6 overflow-y-auto flex-1 text-base">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-base font-bold text-slate-800 mb-2">Article Title *</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                ...
                <div>
                  <label className="block text-base font-bold text-slate-800 mb-2">Author Role</label>
                  <input
                    type="text"
                    value={formData.authorRole || ''}
                    onChange={(e) => setFormData({ ...formData, authorRole: e.target.value })}
                    className="w-full bg-slate-50 border-2 border-slate-300 rounded-xl px-4 py-3.5 text-base font-bold text-slate-900 focus:outline-none focus:border-cyan-600"
                    placeholder="Editorial & Tech Team"
                  />
                </div>
                <div>
              ...
              <div>
                <label className="block text-base font-bold text-slate-800 mb-2">Article Highlights (1 per line)</label>
                <textarea
                  rows={3}
                  value={highlightsInput}
                  onChange={(e) => setHighlightsInput(e.target.value)}
                  className="w-full bg-slate-50 border-2 border-slate-300 rounded-xl px-4 py-3.5 text-base font-bold text-slate-900 focus:outline-none focus:border-cyan-600 leading-relaxed"
                  placeholder="Key take-away 1&#10;Key take-away 2"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="/assets/blog/blog1.jpg or https://..."
                />
              </div>
